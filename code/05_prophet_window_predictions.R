...
opts_matrix <- expand.grid(end = seq(5, 800, by = 5))
temp_dat <- window_dat
colnames(temp_dat) <- c("ds", "y")

prophet_ar_window_results <- adply(opts_matrix, 1, function(drow){
  begin = 1
  end = drow$end
  duration = 30
  ...
  data.frame(true = test_value, expected = out_val, error = out_val - test_value, upper = upper - test_value, lower = lower - test_value)
  # plot(mmod, forecast)
